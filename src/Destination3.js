...
const Destination3 = (props) => {
    
    const [clicked, setClicked] = useState(0);
    const [selected, setSelected] = useState(0);

    const planetSelectingHandler = (evt) => {
        props.SetDistanceOfDestinationThree((prev) => prev= evt.target.value); setClicked(true); 
    ...
    return (
        ...
        {props.Planets  ?
      <div>
      <select value={props.DistanceOfDestinationThree}  onChange = {planetSelectingHandler}>
  ...
  {props.Planets.filter((planet) =>(planet.distance !== Number(props.DistanceOfDestinationOne)) && (planet.distance !== Number(props.DistanceOfDestinationTwo)) && 
      ...
      return(
      <option value={Item.distance} text={Item.name} key={Idx} disabled = {Number(props.DistanceOfDestinationThree) === Item.distance} >{Item.name}
      </option>)})}
  </select>
  {clicked ? <form >
            ...
            return(
                <div key={idx} style={{textAlign:"left"}}><input type="radio" value={item.max_distance} name={item.name} 
                    checked ={Number(selected) === item.max_distance} onChange={(evt) =>{setSelected(evt.target.value); 
                        props.SetTimeTaken((prev) => prev + (props.DistanceOfDestinationThree / item.speed));   
                    ...
                    disabled ={Number(props.DistanceOfDestinationThree) > item.max_distance || props.NumberOfItems[idx] === 0} />  {item.name}({props.NumberOfItems[idx]})  <br /></div>)})}
            </form> : "" } 
  </div>
    : ""}
    </div>
